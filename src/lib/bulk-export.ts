...
function sheet<T extends object>(rows: T[]) {
  return XLSX.utils.json_to_sheet(rows.length ? rows : [{ Note: "No data" }]);
}
...
export async function bulkExport(opts: {
    ...
    for (const s of stations) {
      ...
      const rows = sTasks.map((t) => {
        ...
        return {
          WBS: t.wbs_code,
          Task: t.name,
          "Plan Start": t.baseline_start,
          "Plan Finish": t.baseline_finish,
          "Actual Start": st?.actual_start ?? "",
          "Actual Finish": st?.actual_finish ?? "",
          "% Comp": st?.percent_complete ?? 0,
    ...
    for (const s of stations) {
      for (const b of (master ?? []).filter((m) => m.station_id === s.id)) {
        ...
        rows.push({
          ...
          SL: b.sl_no,
          BOI: b.name,
          Drawings: b.drawings_count ?? "",
          "Sched PO": b.scheduled_po_date ?? "",
          "Actual PO": r?.actual_po_date ?? "",
  ...
  if (opts.reports.includes("delays")) {
    ...
    const rows = (data ?? []).map((r) => ({
      Station: sName.get(r.station_id) ?? "",
      Title: r.title,
      Reason: r.reason_category ?? "",
      "Root Cause": r.root_cause ?? "",
      Responsibility: r.responsibility ?? "",
      "Corrective Action": r.corrective_action ?? "",
      "Recovery Plan": r.recovery_plan ?? "",
      "Recovery Date": r.recovery_date ?? "",
      Status: r.status,
    ...
    for (const s of stations) {
      for (const c of master ?? []) {
        ...
        rows.push({
          ...
          Status: r?.status ?? "not_applied",
          Applied: r?.application_date ?? "",
          Approved: r?.approval_date ?? "",
          Expires: r?.expiry_date ?? "",
          "Doc Ref": r?.document_ref ?? "",
          Owner: r?.owner ?? "",
          Remarks: r?.remarks ?? "",
        });
      }
  ...
  if (opts.reports.includes("audit")) {
    ...
    const rows = (data ?? []).map((r) => ({
      When: r.created_at,
      User: r.user_email ?? "",
      Station: r.station_id ? (sName.get(r.station_id) ?? "") : "",
